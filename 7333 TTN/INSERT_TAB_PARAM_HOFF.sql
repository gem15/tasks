PROCEDURE INSERT_TAB_PARAM_HOFF IS      --<- yurmir *** 12.08.2018 *** Pz 6707
  CURSOR TIR(ID_TIR_IN VARCHAR2) IS 
    select distinct a.id_pok ,a.ID_ZAK, count(*) quantity, 1 /*a.n_zakaza*/ n_zakaza
    from   kb_spros a
  ...
  str_NOMER_ADZ varchar2(1000);
  str_n_zakaza varchar2(1000);
  ...
  --  	
BEGIN
-->	alert_note(' Hoff prc -> TYPE !!! ->'||:GLOBAL.v_TTN);     --<- yurmir *** 08.08.2018 
    ...
    Get_Connect_Info(the_username,the_password,the_connect);
  	...
  	FOR I IN TIR(ID_TIR_IN) LOOP
  	  ...
  	  FOR Y IN 	N_ZAKAZ( I.id_pok, ID_TIR_IN ) LOOP 
		    ...
		    str_n_zakaza := Y.N_ZAKAZA||' / '||str_n_zakaza;       --<- Сборка номеров заказов *** yurmir  *** 09.08.2018 
  	    begin
  	  	  v_marker := '5';
		...
		str_NOMER_ADZ := v_n_adz||' / '||str_NOMER_ADZ;        --<- Сборка номеров АДЗ *** yurmir  *** 09.08.2018 
  	  	--
    	END LOOP;
        --  
--> VVV <-- pz6707 *** yurmir *** 12.08.2018 
        -- определяю к-во грузов
        v_marker := '10_01'; 
  	   BEGIN
			select count(*) into v_pm_quant 
			from loads@wms3 
			where order_id in (
				select distinct id_du from kb_sost where id_obsl in (
					-----------------------------------------------------------------------
					select id from kb_spros where id_tir = ID_TIR_IN and id_pok = I.id_pok
					-----------------------------------------------------------------------
				) AND ID_DU IS NOT NULL AND ID_SOST = 'KB_USL60175'
			);
  	   EXCEPTION 
  	   	 WHEN NO_DATA_FOUND THEN
							v_pm_quant:=null;  
  	   END ;
--> ^^^ <-- pz6707 *** yurmir *** 12.08.2018 
	   -- определяю название типового груза
        v_marker := '10'; 
  	   ...
  	   EXCEPTION 
  	   	 WHEN NO_DATA_FOUND THEN
  	   	    -- gem 28.05.2021ALERT_NOTE('Внимание отсутствует название Типового Груза' );
  	   ...
  	   v_marker := '15';
  		  INSERT /*+ APPEND */ INTO KB_T_PRINT_TN(
  		            ...
  		            VALUES   (
  		                       ...
  		                       ID_TIR_IN   ,            --id_tir           ,   
  		                       str_n_zakaza,            --i.n_zakaza , --v_n_zakaza  , --N_ZAKAZA ,   
  		                       I.ID_ZAK    ,            --ID_ZAK           ,   
  		                       v_tip_gruz  ,            --tipovoi_gruz     ,   
  		                       str_NOMER_ADZ ,          --v_n_adz_out ,            --nomer_adz        ,   
...
EXCEPTION when v_exit then Def_KEY_EXIT; 
	WHEN OTHERS THEN
		Dlg.Msg('*Ошибка в форме <' || :system.current_form || '>' || CHR(10) || error_type	 || '-' ||	 error_code  ||	': ' ||	error_text);
END;